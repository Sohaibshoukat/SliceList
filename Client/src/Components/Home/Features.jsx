        ...
        <div className="w-[100%] md:w-[90%] xl:w-[80%] m-auto my-20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-[80%] md:w-[90%] lg:w-auto m-auto">
                {FeaturesData.map((item, index) => (
                    ...
                    >
                        <div className='bg-accence p-3 group-hover:bg-black group-hover:text-white rounded-full inline-block mb-6'>
                            <item.Icon className='text-black group-hover:text-white text-4xl' />
                        </div>
                        <h1 className='text-black text-3xl text-center lg:text-3xl xl:text-4xl font-bold mb-2 font-head'>{item.heading}</h1>
                        <p className='text-black text-lg md:text-lg text-center'>{item.desc}</p>
                    </div>
                ))}
            </div>
        </div>
